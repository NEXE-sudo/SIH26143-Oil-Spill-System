...
from skimage.restoration import denoise_nl_means # The specific algorithm chosen to remove radar speckle
...
def preprocess_scene(
    ...
    for i, band in enumerate(denoised):
        # Exclude land (zeros) and any NaN from percentile calc
        valid = band[band > 0] if land_mask is not None else band.ravel()
...
if __name__ == "__main__":
    ...
    print(f"✅ Saved: {out}")
    arr = np.load(out)
    print(f"   Shape: {arr.shape} | Range: [{arr.min():.4f}, {arr.max():.4f}] | Dtype: {arr.dtype}")